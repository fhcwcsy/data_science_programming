...
Before generating TDM, I wrote another crawler to grab the names of popular political figures from [here](https://dailyview.tw/top100/topic/9?volumn=1&page=1):

```{r, code = readLines("./names.R")}
```
...
```{r, code = readLines("./place.R")}
```
Now, I can use `jieba` to cut the files into words and generate TDM.

```{r, code = readLines("./text_mining.R")}
```

### Calculate TF_IDF
```{r, code = readLines("./tfidf.R")}
```
